...
+function ($) {
    ...
    $.fn.dcalendarpicker = function (opts) {
        return $(this).each(function () {
            var that = $(this);
            var exs = [8, 9, 46, 116, 123];
            var cal = $('<table class="calendar"></table>'), hovered = false, selectedDate = false,
                w1 = that.outerWidth(true), w2 = that.width();
            that.wrap($('<div style="display:inline-block;position:relative;width:' + w1 + 'px;"></div>'));
            that.css({width: w2});
            cal.css({
                position: 'absolute',
                left: 0, display: 'none',
            ...
            that.on('keydown', function (e) {
                if (exs.indexOf(e.keyCode) >= 0) return true;
                else if (e.which) return false;
            }).on('focus', function () {
				$('div.datepicker').find('.calendar').not(cal).hide();
				cal.show();
